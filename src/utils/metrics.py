...
class MetricsCollector:
    ...
    def _setup_prometheus_metrics(self):
        ...
        self.active_connections = Gauge(
            "oasis_active_connections", "Number of active connections", ["connection_type"]
        )
        ...
        self.memory_usage = Gauge(
            "oasis_memory_usage_bytes", "Memory usage in bytes", ["component"]
        )

        # Error metrics
        self.error_count = Counter(
            "oasis_errors_total", "Total number of errors", ["error_type", "component"]
        )

        # Ethics audit metrics
        self.ethics_audit_count = Counter(
            "oasis_ethics_audits_total", "Total number of ethics audits", ["result"]
        )

        # Info metrics
        self.app_info = Info("oasis_app", "Application information")
        self.app_info.info({"version": "1.0.0", "name": self.app_name})
    ...
    # Diagnosis tracking
    def track_diagnosis(
        self, diagnosis_class: str, confidence: float, approved: bool, duration: float
    ):
        """Track diagnosis result"""
        if PROMETHEUS_AVAILABLE:
            self.diagnosis_count.labels(
                diagnosis_class=diagnosis_class, approved=str(approved)
            ).inc()
            self.diagnosis_duration.observe(duration)
            self.diagnosis_confidence.labels(diagnosis_class=diagnosis_class).observe(
                confidence / 100.0
            )
        else:
    ...
    # Agent tracking
    def track_agent_execution(self, agent_name: str, duration: float, status: str = "success"):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                collector.record_custom_metric(f"{metric_name}_duration_seconds", duration, labels)
...
class Timer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        collector = get_metrics_collector()
        collector.record_custom_metric(f"{self.name}_duration_seconds", self.duration, self.labels)
