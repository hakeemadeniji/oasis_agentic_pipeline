...
class RealtimeInferenceEngine:
    ...
    async def send_progress_update(
        self, websocket: WebSocket, stage: str, progress: float, message: str
    ):
    ...
    async def stream_diagnosis(
        self, websocket: WebSocket, patient_data: Dict[str, Any], image_base64: Optional[str] = None
    ):
        ...
        try:
            ...
            await self.send_progress_update(
                websocket, "image_processing", 0.1, "Processing MRI image..."
            )
            ...
            if image_base64:
                ...
                await self.send_progress_update(
                    websocket, "vision_analysis", 0.3, "Running Vision Agent analysis..."
                )
                ...
                await websocket.send_json(
                    {"type": "result", "agent": "vision", "data": vision_result}
                )

                await self.send_progress_update(
                    websocket, "explainability", 0.5, "Generating Grad-CAM heatmap..."
                )

                # Generate explainability
                img_tensor.requires_grad = True
                heatmap = self.cmo.explainer_agent.generate_heatmap(
                    img_tensor, target_class=pred_idx
                )
                ...
                await websocket.send_json(
                    {"type": "result", "agent": "explainer", "data": explainability_result}
                )
            else:
            ...
            await self.send_progress_update(
                websocket, "temporal_analysis", 0.6, "Analyzing longitudinal progression..."
            )

            longitudinal_id = patient_data.get("longitudinal_id")
            if longitudinal_id:
                long_metrics = self.cmo.temporal_agent.calculate_progression_trajectory(
                    longitudinal_id
                )
                temporal_result = {
            ...
            await websocket.send_json(
                {"type": "result", "agent": "temporal", "data": temporal_result}
            )

            # Stage 3: RAG Query
            await self.send_progress_update(
                websocket, "rag_query", 0.75, "Retrieving clinical guidelines..."
            )
            ...
            # Send RAG results
            await websocket.send_json(
                {"type": "result", "agent": "rag", "data": {"context": rag_context}}
            )

            # Stage 4: Ethics Audit
            await self.send_progress_update(
                websocket, "ethics_audit", 0.9, "Performing ethics and safety audit..."
            )
            ...
            # Send ethics results
            await websocket.send_json({"type": "result", "agent": "ethics", "data": ethics_result})

            # Stage 5: Final Diagnosis
            await self.send_progress_update(
                websocket, "finalization", 1.0, "Finalizing diagnosis..."
            )
            ...
            await websocket.send_json({"type": "final", "data": final_diagnosis})
    ...
    async def stream_batch_diagnosis(self, websocket: WebSocket, patients: list):
        ...
        for idx, patient_data in enumerate(patients):
            try:
                ...
                await self.stream_diagnosis(
                    websocket, patient_data, patient_data.get("image_base64")
                )
...
class StreamingInferenceAPI:
    ...
    async def stream_diagnosis_sse(
        self, patient_data: Dict[str, Any], image_base64: Optional[str] = None
        ...
        try:
            ...
            if image_base64:
                ...
                img = Image.open(BytesIO(image_data)).convert("L")
                img_tensor = (
                    self.engine.cmo.image_transform(img).unsqueeze(0).to(self.engine.cmo.device)
                )
                ...
                yield format_sse({"type": "result", "agent": "vision", "data": vision_result})
            else:
                vision_result = {"class": "Not provided", "confidence": 0.0}
            ...
            longitudinal_id = patient_data.get("longitudinal_id")
            if longitudinal_id:
                long_metrics = self.engine.cmo.temporal_agent.calculate_progression_trajectory(
                    longitudinal_id
                )
                temporal_result = {
                    "trend": long_metrics.get("clinical_trend", "N/A"),
            ...
            yield format_sse({"type": "result", "agent": "temporal", "data": temporal_result})
...
async def websocket_diagnosis_handler(
    websocket: WebSocket, engine: RealtimeInferenceEngine, client_id: str
):
