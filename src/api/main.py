...
from api.validators import sanitize_patient_id, validate_image_upload
...
class PatientData(BaseModel):
    ...
    @classmethod
    def _sanitize_patient_id(cls, v: str) -> str:
        # Reuse the shared sanitizer (charset + length); raises ValidationError(400).
...
class DiagnosisRequest(BaseModel):
    ...
    @field_validator("longitudinal_id")
    @classmethod
    def _sanitize_longitudinal_id(cls, v):
        return sanitize_patient_id(v) if v else v

    class Config:
        json_schema_extra = {
            "example": {
                "patient_data": {"patient_id": "OAS2_0001", "age": 75.5, "mmse": 24.0},
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "biomarker_analysis": {"age_risk": "elevated", "mmse_category": "mild_impairment"},
                "temporal_analysis": {
                    "trend": "Typical Age-Related Neuro-Degradation",
                    "atrophy_velocity": 0.45,
                },
                "rag_context": ["Clinical guideline text..."],
                "explainability": {"heatmap_available": True, "peak_activation": [112, 98]},
                "ethics_audit": {"approved": True, "message": "All safety checks passed"},
...
async def startup_event():
    """Initialize CMO and security on startup"""
    global cmo
    print("Initializing OASIS Agentic Pipeline...")
...
async def diagnose_patient(
    ...
    current_user: Any = Depends(get_current_user_optional),  # Optional auth
):
    """
    ...
    try:
        ...
        biomarker_result = {
            ...
            "mmse_category": "severe_impairment"
            if request.patient_data.mmse < 10
            else "moderate_impairment"
            if request.patient_data.mmse < 20
            else "mild_impairment"
            if request.patient_data.mmse < 24
            else "normal",
        }
        ...
        if request.longitudinal_id:
            long_metrics = cmo.temporal_agent.calculate_progression_trajectory(
                request.longitudinal_id
            )
            temporal_result = {
        ...
        ethics_result = {"approved": not is_flagged, "message": restriction_log}

        # Regional volumetry (Agent 9): use FreeSurfer aseg when a subject id is given.
        volumetry_dict: Dict[str, Any] = {
            "source": "unavailable",
            "summary": "No FreeSurfer subject id provided.",
        }
        ...
        pet = (
            cmo.pet_pup.analyze_subject(request.longitudinal_id)
            if request.longitudinal_id
            else None
        )
        ...
        # strings (file paths, stack context) are never disclosed to the client.
...
async def diagnose_batch(
    ...
    current_user: Any = Depends(require_clinician),  # Requires clinician role
):
    """
    ...
    async def process_batch():
        ...
        for req in request.requests:
            try:
                ...
                results.append({"patient_id": req.patient_data.patient_id, "error": str(e)})
...
async def upload_image(file: UploadFile = File(...)):
    ...
    """
    try:
        # Read and validate image (extension, size, path-safe filename)
        contents = await file.read()
        validate_image_upload(os.path.basename(file.filename or "upload.png"), len(contents))
        ...
        logger.warning("Image upload rejected", exc_info=True)
        raise HTTPException(status_code=400, detail="Invalid image file")


# Get patient data endpoint
@app.get("/patients/{patient_id}", tags=["Patients"])
async def get_patient(patient_id: str, current_user: Any = Depends(require_clinician)):
    """Get patient data by ID (exposes clinical data — requires clinician role)"""
    if cmo is None:
        raise HTTPException(status_code=503, detail="CMO not initialized")
...
async def list_patients(
    ...
    current_user: Any = Depends(require_clinician),
):
    """List all patients with pagination (exposes clinical data — requires clinician role)"""
    if cmo is None:
        raise HTTPException(status_code=503, detail="CMO not initialized")
...
async def export_results(
    ...
    """
    Export diagnosis results in specified format (requires clinician role).

    - **job_id**: Batch job ID or patient ID
    - **format**: Export format (json or csv)
    """
    # Guard against path traversal / header injection: job_id flows into a filesystem
    ...
    if format == "json":
        return FileResponse(result_file, media_type="application/json", filename=f"{job_id}.json")
    elif format == "csv":
        # Convert JSON to CSV
        import json
        import csv
...
async def cure_research(
    current_user: Any = Depends(require_researcher),  # Requires researcher role
):
    """
...
async def sample_image(
    label: Optional[str] = Query(None, description="Class folder to sample from"),
):
    """
