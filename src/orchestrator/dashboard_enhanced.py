...
# Header
st.markdown(
    '<div class="main-header">🧠 OASIS Multi-Agent Diagnostic System</div>', unsafe_allow_html=True
)
...
with st.sidebar:
    st.image(
        "https://via.placeholder.com/300x100/1f77b4/ffffff?text=OASIS+Pipeline",
    ...
    if mode == "Single Patient Analysis":
        ...
        if search_id:
            matching = cmo.patient_df[
                cmo.patient_df["Subject_ID"].str.contains(search_id, case=False)
            ]
            ...
            patient_idx = st.selectbox(
                ...
                format_func=lambda x: (
                    f"{cmo.patient_df.iloc[x]['Subject_ID']} (Age: {cmo.patient_df.iloc[x]['Age']:.0f})"
                ),
...
if mode == "Single Patient Analysis":
    ...
    with col1:
        st.metric("Patient ID", patient_row["Subject_ID"], delta=f"Index {patient_idx}")
    ...
    with col4:
        risk_level = (
            "High"
            if target_age > 75 and target_mmse < 24
            else "Medium"
            if target_age > 70 or target_mmse < 26
            else "Low"
        )
    ...
    try:
        sample_images = [
            f for f in os.listdir(test_folder) if f.endswith((".jpg", ".jpeg", ".png"))
        ]
    ...
    if image_path and os.path.exists(image_path):
        ...
        if is_flagged:
            st.markdown(
                f'<div class="danger-box"><strong>🚨 CRITICAL ALERT: Diagnosis Blocked</strong><br>{restriction_log}</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<div class="success-box"><strong>✅ Ethics Audit Passed</strong><br>{restriction_log}</div>',
                unsafe_allow_html=True,
            )
        ...
        with tab1:
            ...
            with col_img:
                st.image(
                    image_path, caption=f"MRI Scan: {selected_image_file}", use_container_width=True
                )
        ...
        with tab5:
            ...
            with col1:
                ...
                st.metric(
                    "Atrophy Velocity", f"{atrophy_vel:.3f}%/year", delta="Brain volume loss rate"
                )

            with col2:
                st.metric(
                    "MMSE Drift", long_metrics.get("mmse_drift", "N/A"), delta="Cognitive change"
                )
                ...
                trend_data = {
                    ...
                    "Value": [
                        atrophy_vel * 10,
                        abs(float(long_metrics.get("mmse_drift", 0))),
                        long_metrics.get("visits_tracked", 0),
                    ],
                }
    ...
    if uploaded_file:
        ...
        if st.button("Process Batch"):
            ...
            for i, row in df.iterrows():
                ...
                results.append(
                    {"patient_id": row.get("Subject_ID", f"P{i}"), "status": "processed"}
                )
    ...
    fig.update_layout(
        title="Model Accuracy Over Time", xaxis_title="Date", yaxis_title="Accuracy (%)"
    )
    st.plotly_chart(fig, use_container_width=True)
