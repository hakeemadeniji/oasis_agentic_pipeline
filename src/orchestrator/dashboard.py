...
st.sidebar.info(
    "Modify these values to stress-test Agent 6's Ethical Guardrail Audit mechanisms in real-time."
)
...
if image_path and os.path.exists(image_path):
    ...
    with col1:
        st.metric(
            label="Patient Target Profile",
            value=real_row["Subject_ID"],
            delta=f"Index Map {patient_idx}",
        )
    with col2:
        st.metric(
            label="Chronological Age Vector",
            value=f"{target_age:.1f} Yrs",
            delta="Ingested Tabular" if not override_active else "Sandbox Overridden",
            delta_color="inverse" if override_active else "normal",
        )
    with col3:
        st.metric(
            label="Cognitive Screening Evaluation",
            value=f"{target_mmse:.1f} / 30.0",
            delta="Baseline MMSE" if not override_active else "Sandbox Overridden",
            delta_color="inverse" if override_active else "normal",
        )
    ...
    with left_panel:
        ...
        st.image(
            image_path,
            caption=f"Active Raw Scan Target: {selected_image_file}",
        ...
        st.caption(
            f"**Grad-CAM Feature Activation Target Mapping:** High-density structural feature focus locked to matrix coordinates: {peak_focus}"
        )
    ...
    st.error(
        f"[!] System Error: Failed to cleanly route patient data directories at target location: `{test_folder}`"
    )
