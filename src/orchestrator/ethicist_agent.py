...
class MedicalEthicistAgent:
    ...
    def audit_diagnostic_proposal(
        ...
        if confidence < self.confidence_floor:
            return True, f"REJECTED: Sub-threshold confidence score ({confidence:.2f}%). Minimum required: {self.confidence_floor}%."
        ...
        if mmse_score >= 27.0 and predicted_class in ["Mild Dementia", "Moderate Dementia"]:
            return True, f"REJECTED: Severe Cross-Modal Variance. Model suggested '{predicted_class}' but patient displays near-perfect cognitive function (MMSE: {mmse_score:.1f})."
        ...
        if mmse_score >= 28.0 and atrophy_velocity > 2.0:
            return False, "APPROVED WITH WARNING: Patient is cognitively intact, but displays high-risk structural brain tissue loss (>2.0%/yr). High potential for clinical conversion."
            
        # Rule 4: Critical Failure Check
        if mmse_score <= 12.0 and predicted_class == "Non Demented":
            return True, f"REJECTED: Dangerous Type-II Error Risk. Model classified brain as completely clear, but cognitive scores indicate advanced structural impairment (MMSE: {mmse_score:.1f})."

        return False, "VERIFIED: Comprehensive data streams are mathematically and clinically aligned."
