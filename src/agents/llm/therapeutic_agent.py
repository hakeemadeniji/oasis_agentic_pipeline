...
class TherapeuticInsightAgent:
    ...
    def analyze(self, research_report: Dict[str, Any]) -> TherapeuticResult:
        ...
        if parsed is None:
            return TherapeuticResult(fallback["hypotheses"], fallback["summary"],
    ...
    def _fallback(report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for f in findings:
            ...
            if ("atrophy rate" in blob) or ("brain volume faster" in blob) or ("therapeutic endpoint" in blob):
                hyps.setdefault("Slow whole-brain atrophy rate as a disease-modification endpoint", {
            ...
            if ("cognitive reserve" in blob) or ("education" in blob):
                hyps.setdefault("Cognitive reserve as a modifiable prevention lever", {
            ...
            if ("amyloid" in blob) or ("tau" in blob) or ("atn" in blob):
                hyps.setdefault("Target amyloid/tau pathology (ATN A/T axes)", {
        ...
        return {
            "hypotheses": ranked,
            "summary": ("Deterministic hypothesis set derived from the OASIS associations "
