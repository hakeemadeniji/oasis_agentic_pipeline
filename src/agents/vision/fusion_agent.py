...
class AlzheimerMultimodalFusionNet(nn.Module):
    ...
    def __init__(self, num_classes: int = 4, tabular_dim: int = 8, embed_dim: int = 512):
        super().__init__()
        print(
            f"[*] Constructing Explicit Cross-Attention Fusion Architecture (Embedding: {embed_dim})..."
        )
        ...
        self.classifier = nn.Sequential(
            nn.Linear(embed_dim, 256), nn.ReLU(), nn.Dropout(0.3), nn.Linear(256, num_classes)
        )
