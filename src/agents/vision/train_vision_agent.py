...
class VisionAgentTrainer:
    ...
    def __init__(
        self, data_dir: str, epochs: int = 10, batch_size: int = 32, learning_rate: float = 1e-4
    ):
        self.epochs = epochs
    ...
    def train_model(self, save_dir: str):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_idx, (images, labels) in enumerate(self.train_loader):
                ...
                if batch_idx % 50 == 0:
                    print(
                        f"    Epoch [{epoch + 1}/{self.epochs}] Batch [{batch_idx}/{len(self.train_loader)}] Loss: {loss.item():.4f}"
                    )
