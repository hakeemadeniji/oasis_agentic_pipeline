...
class VisionAgentTrainer:
    ...
    def _create_scheduler(self):
        ...
        if self.config.lr_scheduler == "step":
            ...
            return optim.lr_scheduler.ReduceLROnPlateau(
                self.optimizer, mode="min", factor=self.config.lr_gamma, patience=5
            )
        else:
            return None
    ...
    def train_epoch(self) -> Tuple[float, float]:
        ...
        pbar = tqdm(
            self.train_loader, desc=f"Epoch {self.current_epoch + 1}/{self.config.num_epochs}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train Vision Agent for Alzheimer's Classification"
    )
