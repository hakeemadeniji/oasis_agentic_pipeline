...
class CrossValidationPipeline:
    ...
    def _create_kfold(self):
        ...
        if self.stratified:
            ...
            return StratifiedKFold(
                n_splits=self.n_folds, shuffle=self.shuffle, random_state=self.config.seed
            )
        else:
            return KFold(n_splits=self.n_folds, shuffle=self.shuffle, random_state=self.config.seed)

    def _create_data_loaders(
        self, train_indices: List[int], val_indices: List[int]
    ...
    def train_fold(self, fold: int, train_loader: DataLoader, val_loader: DataLoader) -> Dict:
        ...
        for epoch in range(self.config.num_epochs):
            ...
            if (epoch + 1) % 5 == 0:
                print(
                    f"Epoch {epoch + 1}: Train Acc: {epoch_train_acc:.2f}%, Val Acc: {epoch_val_acc:.2f}%"
                )
    ...
    def _save_aggregate_results(self, aggregate_results: Dict):
        ...
        print("=" * 70)
        print(
            f"Mean Validation Accuracy: {aggregate_results['mean_val_acc']:.2f}% ± {aggregate_results['std_val_acc']:.2f}%"
        )
        ...
        print(
            f"Mean Validation Loss: {aggregate_results['mean_val_loss']:.4f} ± {aggregate_results['std_val_loss']:.4f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="models/cross_validation",
    ...
    parser.add_argument(
        "--stratified", action="store_true", default=True, help="Use stratified K-Fold"
    )
    ...
    cv_pipeline = CrossValidationPipeline(
        config=config, n_folds=args.n_folds, stratified=args.stratified
    )
