...
def export_and_quantize_multimodal_model(workspace_root: str):
    ...
    weights_path = os.path.join(
        workspace_root, "src", "pipeline", "onnx_inference", "best_vision_agent.pth"
    )
    if os.path.exists(weights_path):
        ...
        print(
            f"[!] WARNING: Checkpoint not found at {weights_path}. Exporting with randomized blueprint weights."
        )
    ...
    quantize_dynamic(
        model_input=raw_onnx_path, model_output=quant_onnx_path, weight_type=QuantType.QUInt8
    )
    ...
    print(
        f" -> Quantized Model Size : {quant_size:.2f} MB (~{((raw_size - quant_size) / raw_size) * 100:.1f}% reduction)"
    )
