...
class HyperparameterTuner:
    ...
    def __init__(
        ...
        self.study_name = study_name or f"vision_agent_tuning_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def objective(self, trial: Trial) -> float:
        ...
        config_params = {
            ...
            'lr_scheduler': trial.suggest_categorical('lr_scheduler', ['step', 'cosine', 'plateau']),
            'lr_step_size': trial.suggest_int('lr_step_size', 5, 20) if trial.params.get('lr_scheduler') == 'step' else 10,
            'lr_gamma': trial.suggest_float('lr_gamma', 0.1, 0.5),
            
            # Data augmentation
            'use_augmentation': trial.suggest_categorical('use_augmentation', [True, False]),
            'rotation_degrees': trial.suggest_int('rotation_degrees', 5, 30) if trial.params.get('use_augmentation') else 15,
            'horizontal_flip': trial.suggest_categorical('horizontal_flip', [True, False]) if trial.params.get('use_augmentation') else True,
