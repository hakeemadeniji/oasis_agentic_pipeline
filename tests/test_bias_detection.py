...
class BiasDetector:
    ...
    def calculate_equalized_odds(self, attribute: str) -> Dict[str, float]:
        ...
        for group in groups:
            ...
            true_positives = (
                (group_data["true_label"] == "Dementia")
                & (group_data["predicted_label"] == "Dementia")
            ).sum()
    ...
    def generate_bias_report(self) -> Dict:
        ...
        report = {"summary": {}, "detailed_metrics": {}}
        ...
        report["summary"]["overall_fair"] = all(
            metrics["overall_fair"] for metrics in report["detailed_metrics"].values()
        )
        report["summary"]["avg_fairness_score"] = np.mean(
            [metrics["fairness_score"] for metrics in report["detailed_metrics"].values()]
        )
    ...
    sample_data = pd.DataFrame(
        {
            ...
            "predicted_label": np.random.choice(
                ["Non Demented", "Dementia"], n_samples, p=[0.6, 0.4]
            ),
            "confidence": np.random.uniform(0.5, 1.0, n_samples),
        }
    )
