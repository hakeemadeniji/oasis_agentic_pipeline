...
class TestHeatmapNormalization:
    ...
    def test_min_max_normalization(self, explainer_agent, sample_input):
        ...
        # Min-max normalization always yields values in [0, 1]. The max equals 1
        # only when the map is non-constant; an untrained model on random input can
        # produce an all-zero (post-ReLU) map that normalizes to all zeros, so the
        # max==1 check is conditional (this is what made the test RNG-flaky before).
        assert heatmap.min() >= 0.0 - 1e-6
        assert heatmap.max() <= 1.0 + 1e-6
        if not np.allclose(heatmap, heatmap.flat[0]):
